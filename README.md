...
| **v0.8.2** | In progress | UX/UI polish pass from community review (#38): one-time ctrl-click link hint + "Reset hints"; "Vault" area tab and "Logs" naming, auto-hidden until recording is enabled; [+] new-tab button next to the last tab; one visual language for active tabs (nav tabs and pinned chips share the session-tab gradient); honest update checks (real network errors + inline Retry instead of a false "up to date"); confirmation on Logs "Clear all"; clickable vault statistics with a Logs count; reveal/eye toggles on all hidden fields; ended cloud (ECS/kubectl) sessions no longer leave a silently dead pane, the tab reconnects on select; ECS tab titles use the service name; opt-in session logging / connection history defaults; fix: IME / CJK input was blocked in the terminal (the focused pane now requests the IME on each redraw, composition is forwarded to the active session, candidate popup follows the caret) |
