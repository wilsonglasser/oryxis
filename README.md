...
| **v0.7** | Planned | P2P sync delivery (engine actually spawns, tombstone-driven delete propagation, two-sided pairing handshake; LAN-only for v0.7, cross-network signaling deferred), Kubernetes provider (`kubectl exec` into pods, namespace + label discovery), Google Cloud provider (Compute Engine + GKE), Azure provider (VMs + AKS), split panes, biometric unlock, custom themes, macOS bundling, Windows ConPTY local shell |
