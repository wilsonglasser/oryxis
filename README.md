...
| **v0.8.2** | **Released** | UX/UI polish pass from community review (#38): one-time ctrl-click link hint + "Reset hints"; "Vault" area tab and "Logs" naming; [+] new-tab button next to the last tab; one visual language for active tabs; honest update checks with inline Retry; confirmations on every destructive action (Clear all, delete recording, plugin uninstall) with a consistent dialog layout; clickable vault statistics; reveal/eye toggles on hidden fields; ended cloud sessions reconnect on select; ECS tab titles use the service name; opt-in session logging / connection history. Security: session recordings scrub secrets/PII (keys, tokens, connection-string credentials, CPF/CNPJ, cards, emails) and are sealed at rest; Ed25519-signed app updates verified before install; SFTP recursive downloads validate server names; master-password changes re-encrypt every secret; per-key-type known hosts. Performance: vault key derived once at unlock (no more per-field Argon2 stalls); coalesced terminal output + cheaper rendering; closed tabs really close their SSH sessions; concurrent sync and AWS discovery. Logs: colorized session viewer with the terminal palette, clickable rows, retention setting (auto-delete by age). Fixes: pinned ECS tabs survive task recycling and reconnect to the live task; pinned tabs no longer duplicate; IME / CJK input works in the terminal |
| **v0.8.3** | In flight | **Home / vault redesign.** Single top-bar layout: the Classic sidebar and the `Layout mode` (Classic / Workspace) setting are retired for a **Navigation** orientation (Horizontal pill strip, default, or a **Vertical icon rail** on the left of the vault content), with Classic users migrated to the rail; the rail scrolls, pins Settings, expands to labels with hover tooltips when collapsed, and uses a thin hover-revealed scrollbar. **Keyboard navigation** on the dashboard: Tab / arrows move a cyclic selection across groups then hosts (2-D by row/column in grid mode, by record in list mode), Enter opens a group or connects a host (or the top result while searching), Escape clears; the search auto-focuses on Home, the selection blurs the input and clears on click, and the selected card scrolls into view. **Host-group icon/colour editor** as a sidebar panel (folders were rename-only). **Search on Cloud Accounts and Proxies**; **Proxies editor** moved to a right-hand sidebar panel; side editor panels rise full-height over the sub-nav. **Per-card accent wash** in each card's own colour (toggle), with a matching gradient accent wash on the top bar (separate toggle). **History-style list mode** (gapped rounded rows, groups + hosts uniform). Shared empty-state pattern across Proxies / Known Hosts / History; empty views hide their toolbar search + New action; the vault switcher is hidden with a single vault; Cloud cards gain the accent hover border |
| **v0.8.4** | Planned | Terminal quick wins (Tabby-inspired): **careful paste** (a multi-line paste warning with a line-count preview to stop a hidden trailing newline from auto-executing, plus bracketed-paste support that honours the remote app's `\e[?2004h` mode, with an opt-out for power users); **tab rename** (a transient per-tab name for the tab's own lifetime, not persisted to the host; tab colour is already the host colour, so no separate colour picker); **tab bar position** (an opt-in setting to dock the tab strip at the bottom of the window) |
| **v0.9** | Planned | Google Cloud provider (Compute Engine + GKE); Azure provider (VMs + AKS); **Telnet protocol** (a per-host protocol selector: SSH stays the default, Telnet is an opt-in for legacy gear that speaks nothing else, network switches / routers, serial-console terminal servers, embedded / industrial devices, with a reduced editor for username / password only and a cleartext note; raw TCP plus RFC 854/855 IAC option negotiation, NAWS window-size and `xterm-256color` terminal-type, no keys / jump / proxy / SFTP / forwards); **serial port** (a third `TerminalTransport` alongside SSH and Telnet, riding the same reduced-editor pattern: device path plus baud / data bits / parity / stop bits, for direct console cables); **RDP / VNC over SSH in one click** (a port-forward rule or host opens the OS-native remote-desktop client through an auto-provisioned `-L` tunnel: `mstsc` on Windows, FreeRDP / Remmina on Linux, Microsoft Remote Desktop on macOS), cross-platform where Bitvise is Windows-only; **command history** (a per-host log of executed commands in the terminal side panel next to Snippets: most-frequent + recent, click to re-insert, with an optional export / live-append to a plain-text session log file for offline reference and sharing); **smart tabs** (built on the same OSC 133 prompt-marker parser as command history: detect the current foreground process and command start / end, notify when a long-running command finishes or when output resumes after a quiet period, with an optional OSC 9;4 progress bar on the tab); biometric unlock; Windows ConPTY local shell; Windows JumpList (recent hosts in the taskbar menu); XChaCha20-Poly1305 wire format (192-bit nonce) on a sync v6 protocol bump |
| **v1.0** | Planned | **Stable 1.0**. Advanced authentication: SSH certificate auth (signed user certs), FIDO2 / security-key keys (`sk-ssh-ed25519`, `sk-ecdsa-sk`), and PKCS#11 / smartcard / YubiKey hardware-backed keys; in-app SSH key generation (Ed25519 / RSA / ECDSA with optional passphrase). Terminal: scrollback search (Ctrl+F find-in-buffer with match highlighting), broadcast / synchronized input across split panes, zmodem / lrzsz in-terminal transfer (send / receive files straight from the prompt with `sz` / `rz`, for the in-a-jump-host case where opening SFTP is awkward), **X11 forwarding** (`-X` / `-Y` trusted / untrusted, opening `x11-req` channels to the local display; works on Wayland desktops via Xwayland, native Wayland / `waypipe` is out of scope), OSC 8 hyperlinks, ad-hoc quick-connect (`user@host` without saving a host), and a command palette. Consistency + stability: a unified form system (shared field / footer / inline-error helpers adopted across every editor) and portable export/import test hardening |
