//! Embedded file-extension -> MIME-type table, generated from the
//! canonical jshttp/mime-db database (the same source `mime_guess` is
//! built from), inverted to map a lowercase extension to one MIME type.
//! Extensions claimed by multiple types are resolved by source priority
//! (iana > apache > nginx). Looked up by binary search. Kept as a plain
//! static array so there is no external dependency.
//!
//! Regenerate from https://github.com/jshttp/mime-db (`db.json`).

/// `(extension, mime)` pairs, sorted by extension for binary search.
static MIME_BY_EXT: &[(&str, &str)] = &[
...
/// MIME type for a lowercase file extension, or `None` if unknown.
pub(crate) fn lookup(ext: &str) -> Option<&'static str> {
    MIME_BY_EXT
        .binary_search_by(|(e, _)| (*e).cmp(ext))
        .ok()
        .map(|i| MIME_BY_EXT[i].1)
}
