...
- **Drag to reorder tabs.** Drag a tab in the strip to reposition it; the other
  tabs slide out of the way live as you cross them. Reordering is scoped to
  within a group (pinned among pinned, normal among normal), so the
  pinned-first layout stays consistent. The pinned order persists across
  restarts.
